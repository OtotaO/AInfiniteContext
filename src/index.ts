...
export class InfiniteContext {
  ...
  constructor(
    ...
    const basePath =
      options.basePath || path.join(os.homedir(), ".infinite-context");
  ...
  public async storeContent(
    ...
    let bucket = Array.from(buckets.values()).find(
      (b) => b.getName() === bucketName && b.getDomain() === bucketDomain,
    );
    ...
    const chunk = await this.memoryManager.createChunk(
      content,
      {
        domain: bucketDomain,
        source: "user-input",
  ...
  public async retrieveContent(
    ...
    // Get the embedding for the query once
    ...
    if (bucketName && bucketDomain) {
      ...
      const relevantBuckets = await this.memoryManager.findRelevantBuckets(
        query,
        3,
      );
  ...
  public async exportChunks(
    ...
    const { exportChunks, ExportFormat } =
      await import("./utils/DataPortability.js");
   ...
   */
  public async verifyChunkIntegrity(
    chunk: Chunk,
    storedHash: string,
  ): Promise<any> {
   ...
   */
  public async estimateIndexMemoryUsage(
    params: any,
    size: number,
  ): Promise<number> {
   ...
   */
  public async getOptimalIndexParams(
    size: number,
    dimension: number,
    memoryBudget?: number,
  ): Promise<any> {
